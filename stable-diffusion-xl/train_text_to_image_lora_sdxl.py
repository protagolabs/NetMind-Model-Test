...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="sd-pokemon-model-lora-sdxl",
...
def main(args):
    ...
    nmp.init_train_bar(total_epoch=args.num_train_epochs, step_per_epoch=len(train_dataloader))
    ...
    epoch = nmp.cur_epoch       
    unet = NetmindDistributedModel(unet)

    logger.info(f'begin training loop')
    steps = 0
    for epoch in range(first_epoch, args.num_train_epochs):
        logger.info(f'training loop {epoch} start')
        unet.train()
        if args.train_text_encoder:
    ...
    logger.info('training loop finish')
